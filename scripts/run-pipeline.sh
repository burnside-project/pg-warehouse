#!/usr/bin/env bash
# =============================================================================
# run-pipeline.sh — Medallion Architecture pipeline orchestrator
#
...
#
# Usage:
#   ./scripts/run-pipeline.sh                  # full pipeline (silver + feat)
#   ./scripts/run-pipeline.sh --silver-only    # silver layer only
#   ./scripts/run-pipeline.sh --feat-only      # feat layer only
#   ./scripts/run-pipeline.sh --preview        # preview feat tables (10 rows)
# =============================================================================
...
CONFIG="${PROJECT_DIR}/pg-warehouse.yml"
...
# Trap: always restart CDC on exit (success or failure)
cleanup() {
    local exit_code=$?
...
#   sql/silver/001_order_enriched.sql → silver.order_enriched
...
# Stop CDC (required for DuckDB single-writer lock)
stop_cdc
...
if [[ "$RUN_SILVER" == "true" ]]; then
    log "=== Silver Layer ==="
    for sql_file in "$PROJECT_DIR"/sql/silver/*.sql; do
        [[ -f "$sql_file" ]] || continue
...
if [[ "$RUN_FEAT" == "true" ]]; then
    log "=== Feat Layer ==="
    for sql_file in "$PROJECT_DIR"/sql/feat/*.sql; do
        [[ -f "$sql_file" ]] || continue
        target=$(target_table_from_path "$sql_file")
